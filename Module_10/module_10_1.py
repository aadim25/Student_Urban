...
def write_words(word_count, file_name):
    ...
    with open('file_name','w', encoding='utf-8') as my_file:
        for i in range(word_count):
            my_file.write(f"Какое-то слово № {i} по порядку>")
            time.sleep(0.1)
        print(f"Завершилась запись в файл {file_name}")
    my_file.close()
    end_time=time.time()
    print('Время вполнения', str(end_time-start_time))
...
print ('Начата работа потоков')
start_time = time.time()
th1 = threading.Thread(target=write_words(10, 'example5.txt'))
th1.start()
th1.join()
th2 = threading.Thread(target=write_words(30, 'example6.txt'))
th2.start()
th2.join()
th3 = threading.Thread(target=write_words(200, 'example7.txt'))
th3.start()
th3.join()
th4 = threading.Thread(target=write_words(100, 'example8.txt'))
th4.start()
th4.join()
end_time = time.time()
print('Время вполнения потоков', str(end_time-start_time))